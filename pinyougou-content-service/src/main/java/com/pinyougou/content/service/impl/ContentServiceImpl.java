...
public class ContentServiceImpl implements ContentService {
	...
	public void update(TbContent content){
		...
		if (content.getCategoryId().longValue()!=oldContent.getCategoryId().longValue()) { //原来的分类id和现在的分类id不相等，则添加到其他广告分类集合中，对添加的集合有影响
			redisTemplate.boundHashOps("content").delete(content.getCategoryId());
		}
	...
	public void delete(Long[] ids) {
		for(Long id:ids){
			redisTemplate.boundHashOps("content").delete(contentMapper.selectByPrimaryKey(id).getCategoryId());//先获取在删除，不然获取不到对象
	...
	public PageResult findPage(TbContent content, int pageNum, int pageSize) {
		...
		public List<TbContent> findByCategoryId(Long id) {
			...
			if (contentList==null) {
				...
				criteria.andStatusEqualTo("1");//获取的必须时有效的广告
